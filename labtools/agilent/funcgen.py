"""
.. module:: agilent.funcgen
   :synopsis: Agilent function generator controller

.. moduleauthor:: Andrej Petelin <andrej.petelin@gmail.com>

This module holds a controller for Agilent function generator.
...
class AgilentFuncGen(AgilentInstrument):
    """This is a controller for Agilent function generator.
    It uses the visa module for IO and it defines a more
    user-friendly interface. Some of the most useful SCPI commands
    have their separate methods. You can of course still use raw
    commands by calling :meth:`~.AgilentFuncGen.ask` or meth:`~.AgilentFuncGen.write`
    ...
    def set_freq(self,freq, units = ''):
        """Sets output frequency in Hz"""
        self.logger.info('Setting frequency to %s %s' % (freq, units))
        self.write('FREQ %s %s' % (freq, units))
        self.check_error()     
                        
    def get_ampl(self):
        """Returns output voltage (VPP) in V"""
        return float(self.query('VOLT?'))
                
    def set_ampl(self,volt, units = ''):
        """Sets output voltage in V"""
        self.logger.info('Setting voltage to %s %s' %  (volt, units))
        self.write('VOLT %s %s' % (volt, units))
        self.check_error() 
    ...
    def set_offset(self, offset = 0., units = ''):
        """Sets offset in V"""
        self.logger.info('Setting offset to %s %s' % (offset, units))
        self.write('VOLT:OFFS %s %s' % (offset, units))
        self.check_error() 
